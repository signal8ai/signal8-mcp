 ...
 * 86 tools are exposed at runtime. The imported register* functions contain
 * 93 active server.registerTool() calls (a further 26 calls in these files are
 * block-commented out behind rights/data gates and are NOT counted); of those
 * 93, 7 are suppressed by the DISABLED_TOOLS set in registerAllTools(), leaving
 * 86. (The extractions/dilution/macro files are not imported at all.) Domains:
 ...
 *     get_sector_etf_snapshot, get_top_movers, get_top_performers
...
// DISABLED: import { registerDilutionTools } from './dilution.js';
...
/**
 * Register the live Signal8 MCP tools on the server (86 exposed at runtime:
 * 93 active server.registerTool() calls in the imported files minus the
...
export function registerAllTools(server: McpServer, client: Signal8ApiClient): void {
  ...
  registerMarketTools(server, client);        // 7 cross-ticker market tools (batch quotes, snapshots, movers, breadth, top-performers)
  ...
  // DISABLED: dilution data is a dead/unreliable feature — get_dilution_risk, get_dilution_performance, get_baby_shelf
  // registerDilutionTools(server, client);
